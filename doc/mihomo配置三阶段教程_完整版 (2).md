...
**流派八：多场景候选列表 + rule-provider 模板锚点（TheFullSmart_test_version 风格）**

这是本教程涉及的最完整的锚点体系，来自 `TheFullSmart_test_version.yaml`（基于 666OS/MihomoPro 和 Lanlan13-14/configfull.yaml 综合优化）。它同时运用了**三类锚点**，每类都有不同的职责：

```yaml
# ── 第一类：组类型锚点（与流派二类似，但新增了 type: smart）─────────────────────────
BaseFB:    &BaseFB    {type: fallback,     interval: 200, lazy: true, url: 'https://www.google.com/generate_204'}
BaseCH:    &BaseCH    {type: load-balance, interval: 200, lazy: true, url: '...', strategy: consistent-hashing, hidden: true}
BaseCR:    &BaseCR    {type: load-balance, interval: 200, lazy: true, url: '...', strategy: round-robin,        hidden: true}
BaseUT:    &BaseUT    {type: url-test,     interval: 200, lazy: true, url: '...', hidden: true}
BaseSmart: &BaseSmart {type: smart,        interval: 200, lazy: true, url: '...', hidden: true, uselightgbm: true}
#                     ↑ 关键新增：smart 类型，用 LightGBM 模型综合延迟+稳定性+历史成功率选节点

# ── 第二类：多场景候选列表锚点（流派六 pr 锚点的升级版）──────────────────────────────
# 核心思路：不同功能场景的候选列表，首位放该场景最适合的地区
...
# ── 第三类：rule-provider 模板锚点（全新！之前所有流派都没有的）───────────────────────
...
rule-providers:
  Github:      {<<: *BehaviorDN, url: https://github.com/666OS/rules/.../GitHub.mrs}
  Advertising: {<<: *BehaviorDN, url: https://github.com/666OS/rules/.../Advertising.mrs}
  TelegramIP:  {<<: *BehaviorIP, url: https://github.com/666OS/rules/.../Telegram.mrs}
  # 几十条规则集，只需换 url，其余全由锚点补全
```
...
```yaml
proxy-groups:
  # 功能组：<<: *SelectXX 控制候选优先顺序，不同场景不同首选
  - {name: 人工智能,  <<: *SelectUS, icon: ...}   # 美国节点最全面，默认首选美国
  - {name: 货币平台,  <<: *SelectSG, icon: ...}   # 新加坡合规，默认首选狮城
  - {name: 国内流量,  <<: *SelectDC, icon: ...}   # 直连优先，不绕路
  - {name: 新闻媒体,  <<: *SelectUS, icon: ...}   # 同样首选美国
  - {name: 油管视频,  <<: *SelectPY, icon: ...}   # 通用代理，无强烈地区偏好

  # 地区策略组（手动 select，用户可切换单个节点，默认指向同地区的智能组）
  - {name: 香港策略, type: select, proxies: [香港智能], include-all: true, filter: *FilterHK, icon: ...}
  #                                          ↑ 第一项是"香港智能"，默认走智能自动选，也可手动点某个节点

  # 地区智能组（hidden，用第一类 BaseSmart + policy-priority 加权）
  - {name: 香港智能, <<: *BaseSmart, filter: *FilterHK, include-all: true, policy-priority: '优:2;中:1;备:0.2', icon: ...}
  #                                                                         ↑ 配合 additional-prefix: '[优] ' 使用
```

**与流派六（liuran001`pr`）的本质区别：**

| 对比项 | 流派六（`pr` 单一候选列表） | 流派八（`Select*` 多场景候选列表） |
|--|--|--|
| 候选列表种数 | 1 种（所有功能组共用同一列表） | 6 种（按场景分别预定义不同首选） |
| AI 服务的默认首选 | 与流媒体相同 | 美国优先（`SelectUS`） |
| 货币平台的默认首选 | 与 AI 相同 | 新加坡优先（`SelectSG`） |
| 国内流量的默认首选 | 可能走代理绕路 | 直连优先（`SelectDC`） |
| 某功能组要换首选 | 必须拆出来单独写，破坏锚点整齐性 | 换一个锚点引用即可（`<<: *SelectHK`→`<<: *SelectUS`） |
| rule-provider 模板 | ❌ 无，每条都要手写 behavior/format/interval | ✅ `BehaviorDN`/`BehaviorIP` 减少几十行重复 |
| 底层自动组类型 | `url-test` | `smart`（`type: smart` + `uselightgbm: true`） |
| 多机场权重区分 | ❌ 所有节点同等权重 | ✅ `policy-priority: '优:2;中:1;备:0.2'` 加权 |

**`type: smart` + `policy-priority` 详解：**

`type: smart` 是 mihomo 独有的组类型，不同于 `url-test` 的纯延迟排序，它综合考虑延迟、稳定性、历史成功率，并通过 LightGBM 机器学习模型持续优化节点选择策略：

```yaml
# policy-priority 配合 additional-prefix 的完整用法
proxy-providers:
  优质服务商:
    url: 'https://airport-a.com/...'
    override:
      additional-prefix: '[优] '   # 所有节点名前加 "[优] " → "[优] 香港 01"
  备用服务商:
    url: 'https://airport-b.com/...'
    override:
      additional-prefix: '[备] '   # 备用机场节点 → "[备] 香港 01"
...
proxy-groups:
  - name: 香港智能
    ...
    # 解读：含"优"关键词的节点权重 2，含"备"的权重 0.2
    # "[优] 香港 01" 的优先级是 "[备] 香港 01" 的 10 倍
    # smart 组在此基础上再叠加历史性能学习，效果远好于单纯 url-test
```

| 对比项 | `url-test` | `smart` |
|--------|------------|---------|
| 选节点依据 | 当前延迟最低者 | 延迟 + 稳定性 + 历史成功率综合 |
| 抗延迟波动 | 弱，瞬时高延迟就切换 | 强，历史数据平滑处理 |
| 重启后表现 | 立即有效 | 需重新积累数据（几分钟~小时级别） |
| `policy-priority` 支持 | ❌ | ✅ 可对不同机场节点加权 |
| 多机场混用 | 只比延迟，不分机场质量 | 配合 `policy-priority` 自动偏向优质机场 |
| 推荐场景 | 节点少、机场质量均一 | 多机场混合、节点多、要求稳定 |

| 优点 | 缺点 |
|------|------|
| 三类锚点分工明确，每类各司其职 | 初始设计工作量大，需提前规划好全部地区组和候选列表名称 |
| `Select*` 按场景设定首选，功能组个性化开箱即用 | 候选列表里的地区组名与实际 group 名必须严格对应，改名时要同步 |
| `BehaviorDN/IP` 锚点让几十条规则集定义极为简洁 | `type: smart` 冷启动效果与 url-test 相近，需运行一段时间才能充分发挥 |
| `type: smart` + `policy-priority` 是目前最智能的节点选择策略 | 三类锚点体系对初学者门槛高，适合已经理解流派二之后再升级 |
...
| **流派八** | 组类型 + 多场景候选列表 + 规则集模板（三类锚点全用） | TheFullSmart（666OS/Lanlan13-14 综合优化） | 短 | ⭐⭐ | ⭐⭐⭐⭐ | ✅ 多机场进阶 |
...
- 想等未来功能：流派五（多 `<<:` 支持后升级）
- 多机场混用、需要按场景设定首选地区、要智能节点选择：流派八（TheFullSmart）
...
| 功能组→策略组→智能底层 | 多机场混合、要求智能节点选择（TheFullSmart） | 最佳 | 中（需设计三类锚点） |

---

**架构四：功能组 → 地区策略组 → 地区智能组（TheFullSmart 三层架构）**

TheFullSmart 在架构三的基础上，在"底层"引入了一个新的分层：每个地区不只有"自动/回退/均衡"三选一，而是把最底层换成 `type: smart` 的智能组，并由上层的手动 `select` 组来调度它：

```
顶层功能组（默认代理、AI、流媒体、电报...）
  ├── <<: *SelectUS → proxies 首位是"美国策略"
  ├── <<: *SelectPY → proxies 首位是"默认代理"
  └── <<: *SelectDC → proxies 首位是"直接连接"
       ↓
地区策略组（香港策略、美国策略...，用户可见，type: select）
  ├── proxies: [香港智能]    ← 默认指向同地区智能组
  ├── include-all: true      ← 同时暴露所有节点供手动选择
  └── filter: *FilterHK      ← 只显示香港节点
       ↓
地区智能组（香港智能...，hidden: true，用户不可见）
  ├── type: smart            ← 不是 url-test，是智能选择
  ├── uselightgbm: true      ← 开启 LightGBM 模型
  ├── policy-priority: '优:2;中:1;备:0.2'  ← 按机场质量加权
  ├── include-all: true
  └── filter: *FilterHK
```
...
```yaml
# 面板里用户看到的"香港策略"是一个 select 组：
# - 默认（第一项）= 香港智能（隐藏的 smart 自动组）
# - 可以手动下拉选一个具体节点，临时覆盖智能选择
- name: 香港策略
  type: select
  proxies: [香港智能]          # 第一项就是智能组，默认走智能选择
  include-all: true            # 同时在面板暴露所有单个节点
  filter: *FilterHK
  icon: ...

# 用户在面板里能看到：
# [✓] 香港智能（默认，实际上是 smart 自动选最佳）
#     香港 01 [优] - 延迟 12ms
#     香港 02 [优] - 延迟 18ms
#     香港 03 [备] - 延迟 35ms
# 当需要特殊场景时（比如某节点解锁特定流媒体），手动切到具体节点
# 平时保持"香港智能"即可全自动
```

与架构三的主要区别在于**底层不是 url-test/fallback/load-balance 三选一，而是固定 smart**，这样牺牲了一些选择灵活性，换来了更智能的节点选择和更简洁的面板（用户不需要在三种自动策略之间纠结）。
...
**来源五：666OS/rules（`.mrs` 全系，域名 / IP 严格分离，细分类目最多）**

这是 TheFullSmart 配置使用的规则集来源，与 MetaCubeX 官方 `.mrs` 的关键区别在于**分类颗粒度更细**，尤其增加了一批国内特色类目：

```yaml
# 特点：
# 1. 全部 .mrs 格式（速度最快）
# 2. 域名规则和 IP 规则严格分离，命名规范（GitHub.mrs vs GitHub.mrs in ip/）
# 3. 包含国内特色分类：LocationDKS（抖音快手书籍定位）、XPTV（IPTV）、Emby 等

# 模板锚点化（配合流派八的 BehaviorDN/IP）
BehaviorDN: &BehaviorDN {type: http, behavior: domain, format: mrs, interval: 86400}
BehaviorIP: &BehaviorIP {type: http, behavior: ipcidr, format: mrs, interval: 86400}
...
rule-providers:
  ...
  # ↑ Emby 媒体服务器，域名+IP 双重匹配（自建服务器走直连/指定节点）

  AppleCN:     {<<: *BehaviorDN, url: https://github.com/666OS/rules/raw/release/mihomo/domain/AppleCN.mrs}
  # ↑ 苹果国内服务（App Store 国区、iCloud 国区），单独直连

  # 标准类目（与其他规则集类似，但全部 .mrs）
  ...
  # 同一个服务同时有域名和 IP 规则，两份都引用才能全覆盖
```

**规则在 `rules:` 段的推荐顺序（666OS 风格）：**
...
rules:
  ...
  # 域名规则（不触发 DNS，处理快）
  - RULE-SET,LocationDKS,抖快书定位    # 国内特色：定位请求直连
  - RULE-SET,Private,直接连接
  - RULE-SET,XPTV,直接连接            # IPTV 直连
  - RULE-SET,AppleCN,直接连接         # 苹果国内服务直连
  - RULE-SET,AI,人工智能
  - RULE-SET,Emby,Emby服              # Emby 专属策略组

  # IP 规则（有 no-resolve 则跳过 DNS）
  - RULE-SET,AdvertisingIP,REJECT,no-resolve
  - RULE-SET,AIIP,人工智能,no-resolve
  - RULE-SET,EmbyIP,Emby服,no-resolve
  - RULE-SET,ChinaIP,国内流量         # 注意：China IP 没有 no-resolve，因为需要解析
```

| 项目 | 说明 |
|------|------|
| 格式 | 全部 `.mrs`（最快） |
| 特色分类 | LocationDKS / XPTV / Emby / AppleCN（其他来源少有） |
| 域名+IP 双覆盖 | 每个主要服务提供 domain.mrs + ip.mrs，覆盖更完整 |
| 对比 SukkA | 类目更贴近国内用户习惯，SukkA 偏向国际化分类 |
| 对比 MetaCubeX 官方 | 颗粒度更细，但维护者是社区个人，需关注仓库活跃度 |
| 适合 | 使用 Emby / IPTV / 国内应用较多、需要精细控制定位域名的用户 |
...
**写法三：`rcode://success` — DNS 层广告拦截（TheFullSmart 独有）**

TheFullSmart 在 `nameserver-policy` 里还用了一个极少数教程提到的特殊值：`rcode://success`。它不是 DNS 服务器地址，而是**直接返回一个"成功但内容为空"的 DNS 响应**，不进行任何实际查询：

```yaml
# 来源：TheFullSmart_test_version.yaml
nameserver-policy:
  # 广告追踪域名：直接在 DNS 层返回空响应，永远解析不到
  "rule-set:Tracking,Advertising": rcode://success
  #                                  ↑ 不查询任何 DNS，直接返回空（NXDomain 的干净版本）
  ...
  # 代理域名用国外 DNS，配合 ECS 和 RULES 参数
  "rule-set:Github,AI,Speedtest,Twitter,Telegram,SocialMedia,Games,Proxy":
    - https://8.8.8.8/dns-query#RULES&ecs=223.5.5.0/24
    - https://cloudflare-dns.com/dns-query
```

**为什么用 `rcode://success` 而不是 `REJECT` 规则？**

```
普通广告拦截流程（只用 rules REJECT）：
  1. APP 发出 DNS 查询（广告域名）
  2. DNS 正常解析，返回广告服务器 IP
  3. APP 连接该 IP
  4. mihomo 规则匹配到 REJECT → 拒绝连接
  5. APP 报错或显示空白

rcode://success 拦截流程：
  1. APP 发出 DNS 查询（广告域名）
  2. mihomo 在 DNS 层直接返回空响应（不查询任何上游 DNS）
  3. APP 解析失败，连接根本不会发起
  → 更早拦截，减少一次网络往返，APP 端更安静
```

| 对比项 | `rules: REJECT` | `nameserver-policy: rcode://success` |
|--------|-----------------|--------------------------------------|
| 拦截时机 | 连接建立阶段 | DNS 解析阶段（更早）|
| 额外网络请求 | DNS 查询仍然发出 | ❌ DNS 查询直接被截断，无外部请求 |
| 依赖 DNS 配置 | 不依赖 | 需要 dns 开启，且域名经过 mihomo DNS |
| 适合场景 | 无 TUN / DNS 劫持 | TUN 模式或 DNS 劫持（dns-hijack）下效果最佳 |
| 与 rules REJECT 关系 | 通常需要同时配置 | 两者互补，`rcode` 先截，`REJECT` 兜底 |

> **使用前提：** `rcode://success` 只在 mihomo 接管了 DNS 解析时才有效（即开启 TUN 模式 + `dns-hijack: ["any:53"]`，或者应用通过 socks5/http 代理且代理侧解析 DNS）。如果系统 DNS 绕过了 mihomo，这个设置不会生效。
